...
analysis_df %>%
  ggplot(aes(y = growth_form, 
             fill = growth_form, col = growth_form)) +
  geom_bar(alpha = 0.5, show.legend = F) #remove legend and reduce opacity of bars
...
lm_growth <- lm(log(stem_diameter) ~ log(height) *
                  growth_form, data= analysis_df)
